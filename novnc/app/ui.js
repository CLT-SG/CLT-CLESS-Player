...
const PAGE_TITLE = "CLEVER - Remote Desktop";
...
const UI = {
    ...
    initSettings() {
        ...
        var host = urlParam.searchParams.get("hostname")
        if (!host) {
            host = 'localhost'
        }
        /* Populate the controls if defaults are provided in the URL */
        document.getElementById('remote-id').innerText = host
        UI.initSetting('host', host);
        UI.initSetting('port', 9001);
        UI.initSetting('encrypt', (window.location.protocol === "http:"));
    ...
    disconnectFinished(e) {
        ...
        if (!e.detail.clean) {
            ...
            if (wasConnected) {
                ...
                UI.showStatus(_("Failed to connect " + UI.getSetting('host').toUpperCase()), 'error');
            }
    ...
    updateFullscreenButton() {
        if (document.fullscreenElement || // alternative standard method
            document.mozFullScreenElement || // currently working methods
            document.webkitFullscreenElement ||
            document.msFullscreenElement) {} else {}
    },
